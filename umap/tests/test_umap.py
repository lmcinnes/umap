...
repetition_dense = np.array([[5,6,7,8],[5,6,7,8],[5,6,7,8], [5,6,7,8],[5,6,7,8],[5,6,7,8],[1,1,1,1], [1,2,3,4], [1,1,2,1]])
...
binary_repeats = np.vstack(
    [np.repeat(binary_data[0:2], [2, 0], axis=0), binary_data, np.zeros((2, 20), dtype='bool')]
)  # Add some all zero data for corner case test.  Make the first three rows identical
...
def repeated_points_large_sparse_spatial():
    model = UMAP(n_neighbors=3, unique=True, force_approximation_algorithm=True).fit(sparse_spatial_data_repeats)
...
def repeated_points_large_dense_spatial():
    model = UMAP(n_neighbors=3, unique=True, force_approximation_algorithm=True).fit(spatial_repeats)
    assert_equal(np.unique(model.embedding_[0:2], axis=0).shape[0],1)
...
def repeated_points_large_sparse_binary():
    model = UMAP(n_neighbors=3, unique=True, force_approximation_algorithm=True).fit(sparse_binary_data_repeats)
    assert_equal(np.unique(model.embedding_[0:2], axis=0).shape[0],1)
...
def repeated_points_large_dense_binary():
    model = UMAP(n_neighbors=3, unique=True, force_approximation_algorithm=True).fit(binary_repeats)
    assert_equal(np.unique(model.embedding_[0:2], axis=0).shape[0],1)
...
def test_sparse_nn_search():
    ...
    init = sparse_initialise_search(
        rp_forest, train.indices, train.indptr, train.data, test.indices,
        test.indptr, test.data,
        int(10 * 6),
        rng_state, spdist.sparse_euclidean, ()
    )
    result = sparse_initialized_nnd_search(
        train.indices, train.indptr, train.data, search_graph.indptr, search_graph.indices, init,
        test.indices, test.indptr, test.data, spdist.sparse_euclidean, ()
    )
