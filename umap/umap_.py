...
import numpy as np
import scipy.sparse
from scipy.sparse import tril as sparse_tril, triu as sparse_triu, dia
...
class UMAP(BaseEstimator):
    ...
    def fit(self, X, y=None):
        ...
        if self.metric == 'precomputed' and self._sparse_data:
            # For sparse precomputed distance matrices, we just argsort the rows to find nearest neighbors
            # To make this easier, we expect matrices that are symmetrical (so we can find neighbors by looking at
            # rows in isolation, rather than also having to consider that sample's column too).
            ...
            for row_id in range(X.shape[0]):
                ...
                # Remove distance from sample to itself (i.e. the matrix diagonal), if present
                row_data = row_data[row_indices != row_id]
                row_indices = row_indices[row_indices != row_id]
                if len(row_data) < self._n_neighbors:
                    raise ValueError("Some rows of the input distance matrix contain fewer than n_neighbors distances!")
